            ...
            <div class="w-100 tl h2 pv3 bt b--black inline-flex items-center">
                <div class="fl w-80 relative f6">
                    ...
                    <ul class="z-999 bg-near-black absolute w-100 pa0 br3 br--bottom mt0 list overflow-auto" style="max-height:9em;top:1.5em">
                        <router-link class="f8 w-100 fl link no-underline pa2 white bt b--white-10" :to="{name:'wallets-send',params:{id:token.ID}}" v-for="(token, index) in tokenList" :key="index">
        ...
        <div class="fl w-100 overflow-hidden" style="height:calc(100% - 50px)">
            ...
            <div class="fl w-100 near-black overflow-y-scroll scrollbar tl" style="height:calc(50vh - 30px)">
                ...
                <div class="pv2 ph3 fl w-50 f6">
                    ...
                    <input type="number" pattern="\d*" @keyup="calcToken" v-model.number="send.Fiat" placeholder="" class="pa2 ba b--orange bg-white-10 fw3 f6  fl near-black pa2 w-100 br2">
                </div>
                
                <div class="pv2 ph3 fl w-50 f6">
                    <small class="tl fl w-100 pv1"> TOKEN AMOUNT TO BE SENT: </small>
                    <input type="number" pattern="\d*" @keyup="calcFiat" v-model.number="send.Token" placeholder="" class="pa2 ba b--orange bg-white-10 fw3 f6  fl near-black pa2 w-100 br2">
                </div>
                    ...
                    <div class="w-100 cf br2 br--btm pv2">
                        <div class="pa1 fl w-100 tc ">
                            <div class="inline-flex items-center">
                                ...
                                <span class="pl2 f2-ns f3">{{humanNumber(send.Token+0)}} <small>{{record.Token.Symbol}}</small> </span>
                            </div>
                        </div>
                        <div class="">
        ...
        methods: {
            ...
            calcFiat(){
                if (this.record.Token.Price > 0){
                    if (this.send.Token > 0){
                        this.send.Fiat = parseFloat(this.send.Token * this.record.Token.Price).toFixed(2)
                    }
                }
            },
            calcToken(){
                if (this.record.Token.Price > 0){
                    if (this.send.Fiat > 0){
                        this.send.Token = parseFloat(this.send.Fiat / this.record.Token.Price).toFixed(2)
                    }
                }
            },
            dropdownTokens(){
                ...
                HTTP.post('/api/accounttokens/search', app.searchTokens,{withCredentials: true}).then((response) => {
                    if (response.data.Body !== null && response.data.Body !== undefined ) {
                        ...
                        if (app.$route.params.id == 0 || app.$route.params.id == undefined){
                            app.$router.push({name:"wallets-send",params:{id:app.tokenList[0].ID}}) 
                            // app.tokenList = [];
